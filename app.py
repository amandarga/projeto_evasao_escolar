...
app.layout = dbc.Container([
    html.H2("🔐 Login com MFA – Dashboard de Evasão Escolar"),
...
def validar_login(n_clicks, usuario, senha, token):
    ...
    if usuario == 'admin' and senha == '123' and token == '654321':
        ...
        return dbc.Alert("Login OK ✅", color="success"), layout_dashboard(), {'display': 'block'}
    else:
        with open("app.log", "a") as f:
            f.write(f"[{datetime.datetime.now()}] Falha Login - Usuário: {usuario}\n")
        return dbc.Alert("Erro na autenticação ❌", color="danger"), "", {'display': 'none'}
